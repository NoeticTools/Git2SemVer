...
public class FileJsonConverter : JsonConverter<File>
{
    public override File Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {
            throw new JsonException("Expected a string value for File path.");
        }

        var path = reader.GetString();
        if (path == null)
        {
            throw new Git2SemVerInvalidFormatException("Expected non-null string for File path.");
        }

        return new File(path);
    }

    public override void Write(Utf8JsonWriter writer, File directory, JsonSerializerOptions options)
    {
